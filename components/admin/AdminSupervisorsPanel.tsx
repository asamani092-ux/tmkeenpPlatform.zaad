  ...
  function toggleNotify(s: Supervisor) {
    ...
    startTransition(async () => {
      ...
      if (!res.ok) {
        setSupervisors((prev) =>
          prev.map((row) =>
            row.id === s.id ? { ...row, notifyOnRegistration: s.notifyOnRegistration } : row
          )
        );
  ...
  return (
    <div className="card space-y-4">
      <div className="flex flex-wrap items-center justify-between gap-2">
        <h2 className="text-xl font-bold text-primary">إدارة حسابات الإدارة</h2>
        {canManage && (
