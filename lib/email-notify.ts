...
/** Audit to process logs (Coolify/Docker) — no ephemeral disk file — O(n). */
async function logEmails(from: string, emails: EmailPayload[]): Promise<void> {
  console.info("[EMAIL LOG]", {
    timestamp: new Date().toISOString(),
    from,
    emails: emails.map((e) => ({ to: e.to, subject: e.subject })),
  });
}
  ...
  await dispatch(params.senderEmail, [
    {
      ...
      body: [
        ...
        "يُرجى الدخول إلى لوحتك وإكمال النموذج في أقرب وقت.",
        `الرابط: ${params.dashboardUrl}`,
        "",
        "مع تحيات فريق منصة تمكين",
      ].join("\n"),
    },
